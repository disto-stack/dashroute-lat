...
Only components with a `.native.tsx` implementation and real interaction have a `.test.tsx` today (`Button`, `Input`, `BottomSheet`, `MissionCard`, `Switch`, `Tabs`, `ProfileMenu`). `turbo.json`'s `test:native` task picks up this script the same way `test` does, so both can run in CI once the pipeline exists.
